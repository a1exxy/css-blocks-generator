...
const { css, items, panel, field, itemStyle: style, setCount } = locators('flexbox');
...
test('override applies only to the selected item and gets its own css rule', async ({ page }) => {
  ...
  await field(page, 'align-self').selectOption('center');
  ...
  expect(text).toMatch(/\.container > :nth-of-type\(2\) \{\s+width: 120px;\s+align-self: center;\s+flex-grow: 2;\s+\}/);
  expect(text).not.toContain(':nth-of-type(1)');
  expect(text).not.toContain(':nth-of-type(3)');
});
...
test('empty overrides emit no rule; clearing removes it; values persist across selection', async ({ page }) => {
  ...
  expect(await css(page).innerText()).toContain(':nth-of-type(1)');
  await items(page).nth(1).click();
...
test('align-self is a select with the valid values, auto is not emitted', async ({ page }) => {
  await items(page).nth(0).click();
  const opts = await field(page, 'align-self').locator('option').evaluateAll((os) => os.map((o) => o.value));
  expect(opts).toEqual(['auto', 'flex-start', 'flex-end', 'center', 'baseline', 'stretch']);
  await expect(field(page, 'align-self')).toHaveValue('auto');
  await field(page, 'align-self').selectOption('flex-end');
  expect(await css(page).innerText()).toContain('align-self: flex-end;');
