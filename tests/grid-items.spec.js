...
test('override applies to selected item only, own css rule', async ({ page }) => {
  ...
  await field(page, 'justify-self').fill('center');
  ...
  expect(text).toMatch(/\.container > :nth-child\(2\) \{\s+width: 120px;\s+order: 2;\s+justify-self: center;\s+\}/);
...
test('spanning via start/end is consistent in preview and output', async ({ page }) => {
  ...
  const text = await css(page).innerText();
  expect(text).toMatch(/:nth-child\(1\) \{\s+grid-column-start: 1;\s+grid-column-end: 3;\s+grid-row-start: 1;\s+grid-row-end: span 2;\s+\}/);
...
test('reducing count drops overrides and selection', async ({ page }) => {
  ...
  await page.locator('#grid-count').fill('2');
  await expect(panel(page)).toBeVisible();
  const text = await css(page).innerText();
  expect(text).not.toContain('nth-child(6)');
  expect(text).toContain(':nth-child(1)');
  await page.locator('#grid-count').fill('6');
  expect(await css(page).innerText()).not.toContain('9px');
  await items(page).nth(5).click();
  await page.locator('#grid-count').fill('3');
  await expect(panel(page)).toBeHidden();
});
